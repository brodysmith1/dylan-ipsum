<script>
  ...
  const copy = async (e, inv) => {
    if (!text.length || !(config.copy ^ inv) || !navigator.clipboard) return
    ...
    unfollowPointer = on(document.body, "pointermove", (e) => followPointer(e, tooltip.element))

    content = text
      .map((song, i) =>
        !(i % 3) && config.headings ? [song.title, format(song.text)] : format(song.text)
      )
      .flat()
      .map((p, i) =>
        config.html ? (i % 4 ? wrap(p, "<p>", "</p>") : wrap(p, "<h2>", "</h2>")) : p
      )
      .join("\r\n\r\n")
  ...
  const write = (e, n, toggleMenu) => {
    ...
    text = new Array(n).fill(0).map(() => find())
...
<aside class:active>
  ...
  {#if !mobile || active}
    <nav>
      <a on:click|preventDefault={() => (modalAbout = true)} href="javascript:;">About</a>
    ...
    <div class="scroller">
      <div class="scroller-inner">
        <div id="inputs">
          ...
          <Input label="years" className="p-4" range="{config.years.min}–{config.years.max}">
            <Slider
              id="years"
        ...
        <div class="toggles flex flex-wrap">
          ...
          <input
            id="in-4"
            type="checkbox"
            bind:checked={config.copy}
            on:click={(e) => copy(e, true)}
          />
  ...
  <article class:highlight>
    {#if !error}
      {#if !text.length}
        <div id="introduction">
          <h2>
            Fill your designs with the greatest lyrics ever written.
            <span class="annotation"
              >And maybe some of the worst. With 677 songs there's bound to&nbsp;be some stinkers.</span
            >
          </h2>
          <hr class="dark" />
          <p>
            Use this tool to generate random text packed with poetic genius. Or just put your Dylan
            knowledge to the test and explore his incredible catalogue in a new way.
          </p>
          <p>
            All lyrics from Bob’s 677 songs are pulled from
            <a href="https://www.bobdylan.com/" target="_blank">bobdylan.com</a> — from 1962’s
            <em>Bob Dylan</em>
            through to 2020’s <em>Rough and Rowdy Ways</em>.
          </p>
          <p>
            So, know your songs well before you start singing. Because something is happening here,
            and it’s time you learn what it is.
          </p>
          <p>
            Also available as an <a
              href="https://github.com/brodysmith1/dylan-ipsum-npm"
      ...
      <button class="btn-red upper" on:click={(e) => write(e, config.n || 10)}>
        {text.length ? "Give me another" + (config.n > 1 ? " " + config.n : "") : "Get born"}
      </button>
    {:else}
      No albums released
      {config.years.max - config.years.min
        ? [config.years.min, config.years.max].join("–")
        : "in " + config.years.max}.
    {/if}
  </article>
</main>
...
{#if false}
  <Modal close={() => (modalSuggest = false)}>
    ...
    <p>
      Now, Bob, I ain’t lookin’ to analyze you, categorize you, finalize you or advertise you. But
      this was a fun project and I am lookin’ to create new ways for people to engage with your
      work.
    </p>
    <p>
      Got an idea for new feature or platform? Just want to say hello? I might be in Tangier, but
      try me:
    </p>
    <hr />
    {#if !submitted}
      <form class="gform" bind:this={form} on:submit={onSubmit}>
        <label type="text"
          >Name
        ...
        <label type="text"
          >Suggestion
          <textarea name="message" rows="4" placeholder="Give me some milk or else go home."
          ></textarea>
        </label>
        {#if formerror}
          <p class="upper" style="color: #ff9; margin: 0 0 .5rem">{formerror}</p>
        {/if}
        <button class="border btn-red upper"> Step it up and go </button>
      </form>
    {:else}
      <div id="formsubmit">
        <p>
          Thanks. Now don’t send me no more letters, no. Not&nbsp;unless&nbsp;you&nbsp;mail them
          from Desolation Row.
        </p>
      </div>
    {/if}
...
{#if modalAbout}
  <Modal close={() => (modalAbout = false)}>
    <p>
      Bob's catalogue includes 136,392 words, 520 mentions of <em>love</em>, 52 mentions of
      <em>law</em>, and 5 mentions of <em>country pie</em>.
      <span style="opacity: 0.5;"
        >(188/677 songs on <a href="https://www.bobdylan.com/songs" target="_blank">bobdylan.com</a>
        have no lyrics.)</span
      >
    </p>
    <p>
      <span class="border">Brody Smith 👨‍💻</span> had a whopping good time building this website. If
      you have any ideas to make this site better, please reach out on his
      <a href="https://brody.fyi/">website</a>.
    </p>
    <p>
      Dylan Ipsum is also available as an <a
        href="https://github.com/brodysmith1/dylan-ipsum-npm"
        target="_blank">npm package</a
      >, if that's your thing.
    </p>
    <a class="bare" download href="/tools/dylan-ipsum/downloads/songs.json"
      ><button class="border" style="margin-top: 1.75rem;"
        >Download json file of all lyrics &darr;</button
      ></a
    >
  </Modal>
{/if}
